...
$empty_data = array( 'form' => '', 'name' => '','lastname' => '','email' => '','phone' => '','subject' => '','message' => '','consent' => '','captcha' => '','captcha_one' => '','captcha_two' => '','url' => '','telephone' => '','fakecheckbox' => '' );
...
$borders = ! empty( $settings['form_borders'] ) ? esc_attr($settings['form_borders']) : 'dark'; 
$form_borders = ' ' . $borders;
...
$captcha_hidden = '<input id="captcha_one-'.$atts_array['id'].'" type="hidden" name="captcha_one" value="'.$captcha_one .'"/><input id="captcha_two-'.$atts_array['id'].'" type="hidden" name="captcha_two" value="'. $captcha_two .'"/>';
...
$basicstyle = 'input, select, textarea {width: 100%;} #captcha-field-'.$atts_array['id'].' { width: 150px; height: intrinsic; } #captcha-question-'.$atts_array['id'].' { width: 80px; height: inherit; cursor: text; border: none !important; outline: none; display: inline-block; background-color: transparent; padding-right: 0; padding-left: 0; box-shadow: none; } #sform-captcha-'.$atts_array['id'].' { width: 50px; border: none !important; outline: none; display: inline-block; background-color: transparent; padding-left: 5px ; box-shadow: none; } input::-webkit-outer-spin-button, input::-webkit-inner-spin-button { -webkit-appearance: none; margin: 0; } input[type=number] { -moz-appearance: textfield; } .carrots { opacity: 0; position: absolute; top: 0; left: 0; height: 0; width: 0; z-index: -1; } .error-des { line-height: 1; color: #dc3545; font-size: 14px; height: 14px; margin-top: 5px; margin-bottom: 5px; } .error-des span { display: none; } .sform-field.is-invalid + .error-des span { display: block; } #errors-'.$atts_array['id'].' { padding: 5px 0 25px 0; position: relative; outline: none; } #errors-'.$atts_array['id'].' span { display: block; visibility: hidden; text-align: center; } .captcha-error { display: none !important; } .noscript { position: absolute; top: 0; width: 100%; } #errors-'.$atts_array['id'].'.top { margin-bottom: 20px; } .form.confirmation { outline: none; } .form.confirmation { text-align: center; padding-top: 50px; } .form.confirmation > img, .form.confirmation > p > img { margin: 30px auto; width: 250px; } #sform-confirmation-'.$atts_array['id'].' { outline: 0; } .d-block { display: inline-block; } .d-none { display: none !important; } .v-visible { visibility: visible !important; } .v-invisible { visibility: hidden !important; opacity: 0; } .align-left { text-align: left; } .align-center { text-align: center; } .align-right { text-align: right; } h1.sform,h2.sform,h3.sform,h4.sform,h5.sform,h6.sform { color: inherit; }';
$style = $stylesheet != 'false' && $cssfile != 'true' ? '<style>'.$basicstyle.'</style>' : '';
$hidden_fields = '<div class="d-none">'.$nonce.$form_id.$style.'<input type="hidden" id="" name="multiple-errors" value="'. $empty_fields .'" /></div><div class="carrots"><label for="url-site'.$isuffix.'">Fake Url</label><input type="text" id="url'.$isuffix.'" name="url-site" value="" tabindex="-1" autocomplete="off" /><br><label for="hobbies'.$isuffix.'">Fake Hobbies</label><input type="text" id="hobbies'.$isuffix.'" name="hobbies" value="" tabindex="-1" autocomplete="off" /><br><input type="checkbox" id="contact-phone'.$isuffix.'" name="contact-phone" value="false" tabindex="-1" autocomplete="off"><label for="contact-phone">Contact me by telephone</label></div>';
...
$captcha_value = $atts_array['id'] == $data['form'] ? esc_attr($data['captcha']) : '';
...
$form_class = $form_direction == 'rtl' ? 'rtl ' . $form_validation . $form_focus . $form_style . $form_borders . $ajax_form : $form_validation . $form_focus . $form_style . $form_borders . $ajax_form;
...
$top_class = $atts_array['id'] != $data['form'] ? ' v-invisible' : '';
