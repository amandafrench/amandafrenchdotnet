...
class SimpleForm_Forms_List extends WP_List_Table  {
	...
	function get_views() {
        ...
        $search_orderby = isset($_REQUEST['orderby']) && in_array($_REQUEST['orderby'], array('name', 'entries', 'date')) ? $_REQUEST['orderby'] : ''; 
	    global $wpdb;
        ...
        if ( $count_published > 0 ) {
         $views['published'] = "<a id='view-published' href='{$published_url }' {$class} >".__( 'Published', 'simpleform' )."</a> (" . $count_published . ")";
        }
    ...
    function column_forwarding($item) {
	    ...
	    if ( esc_attr($item['moveto']) != '0' && esc_attr($item['to_be_moved']) == 'next' && esc_attr($item['onetime_moving']) == '0' ) {
	      global $wpdb;
	      $moveto =  esc_attr($item['moveto']);
          $name = $wpdb->get_var("SELECT name FROM {$wpdb->prefix}sform_shortcodes WHERE id = '$moveto'");
          return $name;
		}
	    else {
          return '-';
       ...
       if ('delete' === $this->current_action()) {
			...
			if ( ! wp_verify_nonce( $nonce, 'delete_nonce' ) ) { $this->invalid_nonce_redirect(); }
             ...
             if (!empty($id)) {
                ...
                delete_transient( 'sform_last_'.$id.'_message' );
			...
			if ( ! wp_verify_nonce( $nonce, 'bulk-' . $this->_args['plural'] ) ) { $this->invalid_nonce_redirect(); }
              ...
              if (!empty($ids)) {
                ...
                foreach( $ids as $form ) { 
                  ...
                  delete_transient( 'sform_last_'.$form.'_message' );
                }
