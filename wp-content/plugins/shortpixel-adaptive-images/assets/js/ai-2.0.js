...
SPAI.prototype.composeApiUrl = function(doRegister, src, w, h, specialCrop) {
    ...
    switch ( ext ) {
        case 'svg':
        case 'webp':
            apiUrl = spai_settings.serve_svg ? ( spai_settings.api_short_url + '/' + src ) : src; //TODO remove after a while - SVG and other non-optimizable resources are by default stored on CDN
            break;
        case 'png':
            apiUrl = apiUrl + ( convType && nextgen.png ? spai_settings.sep + convType : '' ) + '/' + src;
            break;
        case 'jpg':
            apiUrl = apiUrl + ( convType && nextgen.jpg ? spai_settings.sep + convType : '' ) + '/' + src;
            break;
        case 'gif':
            apiUrl = apiUrl + ( convType && nextgen.gif ? spai_settings.sep + convType : '' ) + '/' + src;
            break;
        default: //just to be sure...
            apiUrl = apiUrl + ( convType ? spai_settings.sep + convType : '' ) + '/' + src;
            break;
    }
