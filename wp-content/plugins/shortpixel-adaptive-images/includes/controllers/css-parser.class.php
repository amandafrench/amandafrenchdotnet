...
class ShortPixelCssParser {
    ...
    public function replace_background_image_from_tag($matches, &$tagRule = false, $ignoreLazyNotice = false) {
        ...
        if(($this->ctrl->settings->areas->backgrounds_lazy || $this->ctrl->settings->behaviour->nojquery > 0) && $ret->replaced) {
            ...
            $spaiClass = ($this->ctrl->settings->areas->backgrounds_lazy && !$tagRule->eager ? ' spai-bg-on'
                : ($this->ctrl->settings->behaviour->nojquery > 0  ? ' spai-bg-prepared' : false));

            if($spaiClass) {
                $ret->text = $this->add_class($ret, $spaiClass);
            }
        }
    ...
    public function replace_background_image($matches, $lazy = true, $ignoreLazyNotice = false) {
        ...
        $type = $matches[5];
        $url = trim($matches[6]);
        $q = isset($matches[7]) ? $matches[7] : '';
        $notReplaced = (object)array('text' => $text, 'replaced' => false, 'tag' => $tag);
        ...
        if(   $this->ctrl->urlIsApi($url)
           ...
           || $this->ctrl->urlIsExcluded($url)) {
            $this->logger->log("REPLACE BG - IS INVALID OR EXCLUDED");
            return $notReplaced;
        }

        if(!$lazy || $this->ctrl->tagIs('eager', $text)) {
...
//              . " WITH: " . ' '. $attr . ':' . $extra . 'url(' . $q . $inlinePlaceholder . $q);
        ...
        $replacement =  $attr . $extra . 'url(' . $q . $inlinePlaceholder . $q;
        $str = preg_replace('/' . $attr . preg_quote($extra, '/') . 'url\(\s*' . preg_quote($q . $pristineUrl . $q, '/') . '/',
            $replacement, $text);
