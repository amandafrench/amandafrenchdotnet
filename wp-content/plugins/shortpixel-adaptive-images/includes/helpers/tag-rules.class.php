...
class TagRules
{
    ...
    protected function __construct() {
        ...
        if ( $settings->areas->parse_css_files )
        {
            ...
            $preloadRules = new TagRule('link', 'href', false, 'rel', 'preload', false, true);
            if(!$replaceUrls) $preloadRules->setCustomCompression('orig');
            if($settings->behaviour->nojquery > 0) $ssRule->setPostProcessor([$this, 'addCrossOrigin']);
            $regexItems[] = $preloadRules;
        }
        ...
        elseif(strpos($theme, 'Divi') === 0) {
            ...
            $regexItems[] = new TagRule('img', 'data-et-multi-view', false, false, false, false, false,
                'srcset', 'replace_custom_json_attr');
        }
        elseif($theme == 'Stack') {
        ...
        $extRegexItems = apply_filters( 'shortpixel/ai/customRules', $regexItems);
        $filteredRegexItems = [];
        foreach($extRegexItems as $extItem)
        {
            if($extItem instanceof TagRule) {
                $filteredRegexItems[] = $extItem;
            }
            elseif(is_array($extItem) && count($extItem) >= 2) {
                //valid item
                $filteredRegexItems[] = new TagRule($extItem[0], $extItem[1], isset($extItem[2]) ? $extItem[2] : false, isset($extItem[3]) ? $extItem[3] : false,
                    isset($extItem[5]) ? $extItem[5] : false, false, isset($extItem[6]) ? $extItem[6] : true);
            }
        }
        $logger->log("TAG RULES: ", $filteredRegexItems);
        $this->items = $filteredRegexItems;
    }
