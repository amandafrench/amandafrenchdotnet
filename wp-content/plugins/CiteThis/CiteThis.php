...
function GetCitationsBlockNonStaticHTML($post, $isDynamic, $hasPopup) {
    ...
    if ($isDynamic) // Append dynamic method
        $html .= '<a class="citation-manual-dynamic" onclick="ManualLoad(' . $post->ID . ')">Cite this blog post (current window)</a>&nbsp; |';
    else // Append manual load link
        $html .= '<a class="citation-manual" href="' . get_permalink_citations($post) . '" rel="nofollow">Cite this blog post (current window)</a>&nbsp; | ';

    if ($hasPopup) // Append popup link
        $html .= ' &nbsp;<a class="citation-new-window" href="' . get_permalink_citations_new_window($post) . '" rel="nofollow">Cite this blog post (new window)</a>';
...
function GetDefaultCitationStyles() {
    ...
    $citations['apa']['show'] = true;
    
    $citations['mla']['name'] = 'MLA style';
    $citations['mla']['style'] = '%author%, "%pagename%." <i>%publisher%</i>. %date:j F Y, H:s% UTC. %institution%. %retdate:j M Y% &lt;<a href="%permalink%">%permalink%</a>&gt;.';
    ...
    $citations['mhra']['show'] = true;
    
    $citations['chicago']['name'] = 'The Chicago Manual of Style';
